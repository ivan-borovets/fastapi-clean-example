...
def test_bcrypt_password_hasher_verify(bcrypt_password_hasher):
    ...
    assert (
        hasher.verify(raw_password=wrong_password_vo, hashed_password=hashed) is False
    )
...
def test_bcrypt_password_hasher_with_special_characters(bcrypt_password_hasher):
    ...
    assert (
        hasher.verify(raw_password=special_password_vo, hashed_password=hashed) is True
    )
...
def test_bcrypt_password_hasher_with_different_pepper():
    ...
    assert (
        hasher1.verify(raw_password=test_password_vo, hashed_password=hashed1) is True
    )
    assert (
        hasher2.verify(raw_password=test_password_vo, hashed_password=hashed1) is False
    )
