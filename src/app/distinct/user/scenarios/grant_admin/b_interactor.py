...
class GrantAdminInteractor(InteractorStrict[GrantAdminRequest, GrantAdminResponse]):
    ...
    async def __call__(self, request_data: GrantAdminRequest) -> GrantAdminResponse:
        ...
        :raises GatewayError:
        """
        log.info("Granting admin to user: '%s'.", request_data.username)
        try:
        ...
        log.info(
            "User '%s' is promoted to admin.",
            request_data.username,
        )
        return GrantAdminResponse(request_data.username, ResponseStatusEnum.UPDATED)
